...
class ResidualAttentionNetwork(object):
    ...
    def f_prop(self, x, is_training=True):
        ...
        # print('before x.shape:', x.shape)
        # attention module, x -> [None, row, line, 32]
        x = self.attention_module.f_prop(x, input_channels=32, scope="attention_module_twice_1", is_training=is_training)

        # residual block, x-> [None, row, line, 64]
        x = self.residual_block.f_prop(x, input_channels=32, output_channels=64, scope="residual_block_twice_1",
                                       is_training=is_training)
        # max pooling, x -> [None, row, line, 64]
        x = tf.nn.max_pool(x, ksize=[1, 3, 3, 1], strides=[1, 1, 1, 1], padding='SAME')
        x = x + x_attention_2
        ...
        # print('before x.shape:', x.shape)
        # attention module, x -> [None, row, line, 32]
        x = self.attention_module.f_prop(x, input_channels=32, scope="attention_module_twice_2",
                                         is_training=is_training)

        # residual block, x-> [None, row, line, 64]
        x = self.residual_block.f_prop(x, input_channels=32, output_channels=64, scope="residual_block_twice_2",
                                       is_training=is_training)
        # max pooling, x -> [None, row, line, 64]
        x = tf.nn.max_pool(x, ksize=[1, 3, 3, 1], strides=[1, 1, 1, 1], padding='SAME')
        x = x + x_attention_3
        
        print('x_before_seg_2',x.shape)
        ...
        # x_front_seg_2 = self.seg_module.ConvBnRelu(x_front_seg_1, out_filters=128, kernel_size=3, strides=1,
        #                                            scope='conv_bn_relu_front_2')
        # x_front_seg_3 = tf.nn.max_pool(x_front_seg_2, ksize=[1, 1, 1, 1], strides=[1, 1, 1, 1], padding='SAME')
        # x_front_seg_4 = self.seg_module.ConvBnRelu(x_front_seg_3, out_filters=256, kernel_size=5, strides=1,
        #                                            scope='conv_bn_relu_front_3')
        # x_front_seg_5 = tf.nn.max_pool(x_front_seg_4, ksize=[1, 1, 1, 1], strides=[1, 1, 1, 1], padding='SAME')
        # x_front_seg_6 = self.seg_module.ConvBnRelu(x_front_seg_5, out_filters=512, kernel_size=7, strides=1,
        ...
        x_seg_up2 = self.seg_module.ConvBnReluValid(x_seg_up2, out_filters=128, kernel_size=2, strides=1)
        ...
        x = self.seg_module.ConvBnReluValid(x, out_filters=128, kernel_size=3, strides=1, scope='t_1')
        ...
        # FC, softmax, seg
        tf.add_to_collection('pred_class', y_class)
